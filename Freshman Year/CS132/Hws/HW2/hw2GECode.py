...
def inconsistentSystem(A):
    ...
    #Check if the last element in the last row is a zero
    if A[-1][-1] != 0:
    #If it isn't, check that all other elements in that row aren't zero 
        for y in A[-1][0:-1]:
            y += y
        if y == 0:
    #If they're all zero, then the system is inconsistent
            return True
    return False
...
#check every row and don't stop running until finished
    while row >=0:
        #find non-zero value in rows
        if sum(A[row][0:-1]) != 0:
            for q in range(n):
                #find non-zero value in columns
                if A[row][q] != 0:
                    #assign column value to pivot
                    pivot = q
                    #store coordinates of pivot
                    val = A[row][q]
                    break
                if q == n-1:
                    pivot = q - 1
                    val = A[row][q]
            #turn pivots into 1 by dividing by pivot factor and apply factor to entire row
            for l in range(n):
                A[row][l] = A[row][l]/val
            #Reduce entire row above pivot row
            for u in range(row):
                rowReduce(A, row, u, pivot)
        #move on to next row
        row -= 1
    return A
