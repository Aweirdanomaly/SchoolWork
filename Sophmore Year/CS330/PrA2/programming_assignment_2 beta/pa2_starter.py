...
def dijkstra(N, m, s, adj_list):
    ...
    while (len(nodes) != 0):
        ...
        for x in range(len(distances)):
            if ((distances[x]< minimum) and (x not in seen)):
                minimum = distances[x]
        seen.append(nodes.pop(minimum))

        for adjacent in range(len(adj_list[minimum])):
          
            if (distances[adj_list[minimum][adjacent][0]] > distances[minimum] + adj_list[minimum][adjacent][1]):
                distances[adj_list[minimum][adjacent][0]] = distances[minimum] + adj_list[minimum][adjacent][1]
                parents[adj_list[minimum][adjacent][0]]= minimum
